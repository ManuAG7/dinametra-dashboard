...
const ArrowRight = () => (
  ...
  >
    <path
      d="M7.07404 4.81247H0V3.93749H7.07404L3.75128 0.614729L4.37498 0L8.74996 4.37498L4.37498 8.74996L3.75128 8.13523L7.07404 4.81247Z"
      fill="#FF4D00"
    />
  </svg>
);
...
const LifestyleCardItem = ({ card }: { card: LifestyleCard }) => (
  <div
    className="flex-shrink-0 flex flex-col bg-white overflow-hidden w-full"
    style={{
      borderRadius: 12,
  ...
  >
      ...
      {card.image ? (
        ...
        <div className="w-full h-full flex items-center justify-center">
          <svg width="48" height="48" viewBox="0 0 48 48" fill="none" opacity="0.3">
            ...
            <path d="M8 36L18 22L25 30L30 24L40 36H8Z" fill="white" />
    ...
    <div className="flex flex-col gap-3 p-4 sm:p-5 md:p-6 flex-1">
      <h3 className="text-lg sm:text-xl font-bold text-gray-900 m-0 leading-snug">
        {card.title}
      </h3>
      ...
      <button
        className="flex items-center gap-2 text-[11px] sm:text-xs font-extrabold uppercase tracking-wider mt-auto pt-2 w-fit group outline-none focus:outline-none focus:ring-0"
        style={{
          color: "#FF4D00",
          background: "none",
          border: "none",
...
const Lifestyle = () => {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (width < 768) {
        setVisibleCards(1);
        setCardWidth(Math.min(width - 64, 420));
      } else {
        setVisibleCards(2);
        setCardWidth(405);
      }
  ...
  const VIEWPORT = visibleCards === 1 ? cardWidth : cardWidth * 2 + GAP;
    ...
    >
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 md:px-8 lg:px-16">
        <div className="text-center mb-8 sm:mb-10">
          <h2
            className="text-2xl sm:text-4xl lg:text-5xl font-extrabold m-0 leading-tight"
            style={{ color: "#0F172A" }}
          >
